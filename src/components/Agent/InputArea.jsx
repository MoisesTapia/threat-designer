...
const InputArea = ({inputValue, handleInputChange, handleSendMessage, streaming, handleReasoningSelect, fileValue, setFileValue, sessionEstablished
}) => {
    ...
    const requestPresignedUrl = useCallback((fileId, fileType) => {
      sendMessage({
        action: "agent",
    ...
    useEffect(() => {
      const processNewFiles = async () => {
        ...
        for (const fileItem of newFiles) {
          ...
          setFileValue(prev => prev.map(item => 
            item.file === file ? { ...item, loading: true } : item
          ));
      ...
      processNewFiles();
    }, [fileValue, registerPresignedUrlCallback, requestPresignedUrl, extractS3KeyFromUrl, uploadFile]);
